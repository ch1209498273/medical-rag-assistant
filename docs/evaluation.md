...
### Task 19：正式运行库 80 题复评（2026-09-06）

Task19 将最新 `2026-standard-manual-v1` 从 staging 迁移为独立正式运行库候选，完成本地验收并激活本机运行时。随后使用同一冻结 80 题做一次独立复评，避免把旧 staging 数字直接套到新运行库。

| 指标 | 结果 | 公开解释 |
|---|---:|---|
| 可回答题形成回答 | 57/64（89.06%） | 生成链路工程可用性，不是医学正确率 |
| 无答案安全拒答 | 16/16（100%） | 负例安全门代理 |
| Recall@20 / Recall@6 | 0.607 / 0.595 | 检索命中代理 |
| MRR@20 / nDCG@6 | 0.580 / 0.673 | 证据排序代理 |
| 运行时引用可见 | 57/57（100%） | 引用可展示和可定位，不等于语义正确 |

本轮实际 281 次调用（Embedding 80、Reranker 80、回答 64、语义裁判 57），0 重试；只保存私有聚合，不保存题目、答案、资料正文、Key、headers、reasoning 或 Provider 原始响应。与 Task18-G 的 53/80 差异不能归因于知识库迁移，因为云端生成存在服务波动，检索指标保持一致。完整运行记录只在私有环境保留；公开仓库只记录上述聚合和限制。

### 知识库版本生命周期（公开说明）

源项目已将旧生产索引与旧 staging 运行包移入本地可恢复归档，再把最新的 `2026-standard-manual-v1` 迁移到正式运行库并完成本地验收。公开仓库不包含真实手册、数据库、向量文件或归档内容；历史评测只作为不可篡改的工程追溯证据。公开 Demo 仍使用虚构资料；私有运行库复评不等于公网服务、临床 UAT 或医学质量批准。
