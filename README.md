...
**二期交付状态（2026-09-13）：** 二期已在 `real-local local_demo` 和求职展示范围收口，新增资料/Prompt 生命周期治理、角色与会话隔离、反馈审核与数据飞轮、效果聚合和场景感知检索说明；截图、问题处理思路和三期承接见[二期交付说明](docs/phase2-delivery.md)。公开仓库仍只展示合成资料和脱敏工程证据。
...
二期把项目从“能问答的 Demo”推进到“可治理的医护工作台”：资料草稿/审核/版本、Prompt 草稿/对照/回滚、角色与会话隔离、反馈审核、效果聚合和场景感知检索都形成了可复核的工程边界。公开仓库只展示合成截图和安全说明，真实资料与私有评测留在本机。

二期最典型的处理包括：无证据问题保持拒答并给出单独标记的参考；Enter 默认发送、Shift+Enter 换行；软删除会话但保留 Bad Case 审计；不透明中文 Prompt 弹窗；缺失失效日期的资料只能保留草稿；Hybrid 召回指标提升但回答体验回退时保留 vector 基线；QA10/Q04 输入不完整时阻断并转入三期。完整的截图、问题处理思路和验收边界见[二期交付说明](docs/phase2-delivery.md)。
...
| 配置 | 正式回答可用性 | 无答案拒答 | 结论 |
| --- | ---: | ---: | --- |
| 历史回滚基线/C1（30题） | 9/30（30.0%） | 100% | 历史提示词安全门基线 |
| 历史 C2 实验（30题） | 14/30（46.7%） | 60% | 回答数增加，但低于 90% 安全门，按确认回滚 |
...
2A 是开发验收用的候选工作流实验：A 为当前基线，B 为基线加核验，C 为候选工作流；三者使用同一冻结评测集和同一指标口径。协议、预算封装、零出网预检、失败回退约束和公开导出均已完成。2026-08-31 在单独授权后完成了一次私有 A/C 对照：正式聚合记录 172 次物理调用，A 形成 21/30 个正式回答，C 形成 9/30 个正式回答，其中 C 有 16 题停在 Router 结构校验。随后已在本地加入显式 baseline 回退，并通过 999 个后端测试；结果仍需同协议复评，未自动替换正式路径，默认问答仍为 `baseline_v1`。私有逐题内容不公开，不能把这轮结果当作医疗准确率或生产安全证明。
...
公开副本采用 [MIT License](LICENSE)，仅覆盖独立公开仓库中实际发布的脱敏代码、虚构资料和公开文档；真实资料、私有评测、私人手册和未导出的开发资产不属于该许可范围。
